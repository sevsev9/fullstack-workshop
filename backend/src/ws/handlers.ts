...
export const connectionHandler =
    (wss_server: WsServer) => (ws: WebSocketWithAuth) => {
        ...
        loggerUtil.info(
            "{WebSocket - Connection Handler} - Client has connected",
        );
        ...
        try {
            ...
            loggerUtil.info(
                `{WebSocket - Connection Handler} - User ${ws.user.username} was authenticated successfully.`,
            );

            // send the currently active and joinable lobbies to the user
            const lobbiesList = Object.values(lobbies)
                .filter((e) => e.players.length === 1)
                .map((lobby) => ({ name: lobby.name }));

            ws.send(
                JSON.stringify({
                    type: "lobby_list",
                    payload: { lobbies: lobbiesList },
                } as LobbyListResponse),
            );
        } catch (err) {
            ws.send(
                JSON.stringify({
        ...
        ws.on("close", () => {
            ...
            const lobby = Object.values(lobbies).find((lobby) =>
                lobby.players.some((player) => player.socket === ws),
            );
...
function handleLobbyCreate(ws: WebSocketWithAuth, message: LobbyCreateRequest) {
    ...
    if (lobbies[lobbyName]) {
        sendResponse(ws, {
            type: "lobby_create",
            payload: { success: false, error: "Lobby already exists" },
        });
        return;
    }
    ...
    sendResponse(ws, {
        type: "lobby_create",
        payload: { success: true, lobbyName },
    });
...
function handleLobbyJoin(ws: WebSocketWithAuth, message: LobbyJoinRequest) {
    ...
    if (!lobby) {
        sendResponse(ws, {
            type: "lobby_join",
            payload: { success: false, error: "Lobby does not exist" },
        });
        return;
    }
...
function handleLobbyChat(ws: WebSocketWithAuth, message: LobbyChatRequest) {
    ...
    if (!lobby) {
        sendResponse(ws, {
            type: "lobby_chat",
            payload: { user, lobbyName, message: "Lobby does not exist" },
        });
        return;
    }
